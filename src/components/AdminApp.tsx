...
export default function AdminApp() {
  ...
  const updateStatuses = async (statuses: Record<StatusKind, string>) => {
...
export function LegacyRecordSale({ variants, loading, onSubmit }: { variants: Array<Variant & { productName: string }> | null; loading: boolean; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  const availableVariants = variants ?? [];
...
export function OrderDetail({ order, loading, onBack, onStatuses, onPayment }: { order: AdminOrder; loading: boolean; onBack: () => void; onStatuses: (statuses: Record<StatusKind, string>) => void; onPayment: (event: FormEvent<HTMLFormElement>) => void }) {
  const [draft, setDraft] = useState<Record<StatusKind, string>>({ orderStatus: order.orderStatus, paymentStatus: order.paymentStatus, deliveryStatus: order.deliveryStatus });
  useEffect(() => setDraft({ orderStatus: order.orderStatus, paymentStatus: order.paymentStatus, deliveryStatus: order.deliveryStatus }), [order]);
  return <section className="space-y-5">
    ...
    </div><aside className="rounded-2xl bg-asili-green p-5 text-white xl:sticky xl:top-24 xl:self-start"><h2 className="text-xl">Update progress</h2><form className="mt-6 space-y-5" onSubmit={(event) => { event.preventDefault(); onStatuses(draft); }}>{(["orderStatus", "paymentStatus", "deliveryStatus"] as StatusKind[]).map((kind) => <label key={kind} className="block text-xs font-bold uppercase tracking-wide text-white/70">{label(kind)}<select value={draft[kind]} disabled={loading} onChange={(event) => setDraft((current) => ({ ...current, [kind]: event.target.value }))} className="mt-2 w-full rounded-xl bg-white px-3 py-3 text-sm font-semibold text-asili-green">{statusOptions[kind].map((option) => <option key={option} value={option}>{label(option)}</option>)}</select></label>)}<button disabled={loading} className="w-full rounded-xl bg-asili-honey px-4 py-3 text-sm font-black text-asili-green disabled:opacity-60">Save progress</button></form></aside></div>
  </section>;
}

export function LegacyOrderDetail({ order, loading, onBack, onStatus }: { order: AdminOrder; loading: boolean; onBack: () => void; onStatus: (kind: StatusKind, value: string) => void }) {
  return <section className="space-y-5"><button onClick={onBack} className="inline-flex items-center gap-2 text-sm font-bold"><ArrowLeft className="h-4 w-4" /> All orders</button><div className="flex flex-col items-start gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><Badge value={order.source} /><p className="text-xs font-black uppercase tracking-[0.2em] text-asili-earth">Order detail</p></div><h1 className="mt-2 break-all font-mono text-xl font-bold sm:text-3xl">{order.orderReference}</h1></div>{order.customer.normalizedPhone && <a href={whatsapp(order.customer.normalizedPhone, order.orderReference)} target="_blank" rel="noopener noreferrer" className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#25D366] px-5 py-3 text-sm font-black text-[#073b1a] sm:w-auto">WhatsApp customer <ExternalLink className="h-4 w-4" /></a>}</div><div className="grid gap-5 xl:grid-cols-[1.4fr_1fr]"><div className="space-y-5"><article className="rounded-2xl bg-white p-4 shadow-sm sm:p-5"><h2 className="text-xl">Items</h2><div className="mt-4 divide-y divide-asili-green/10">{order.items.map((item, index) => <div key={`${item.sku}-${index}`} className="grid gap-2 py-4 sm:grid-cols-[1fr_auto]"><div><p className="font-bold">{item.productName} · {item.variantName}</p><p className="mt-1 font-mono text-xs text-asili-green/50">{item.sku ?? "No SKU"} · {item.quantity} × {money(item.unitPriceMinor, order.currency)}</p></div><p className="font-bold sm:text-right">{money(item.lineTotalMinor, order.currency)}</p></div>)}</div><div className="mt-4 space-y-2 border-t border-asili-green/15 pt-4 text-sm"><p className="flex justify-between gap-3"><span>Product subtotal</span><strong>{money(order.subtotalMinor, order.currency)}</strong></p><p className="flex justify-between gap-3"><span>Delivery fee</span><strong className="text-right">{order.deliveryFeeMinor ? money(order.deliveryFeeMinor, order.currency) : "Confirm separately"}</strong></p><p className="flex justify-between gap-3"><span>Payment method</span><strong className="text-right">{order.paymentMethod ?? "Not recorded"}</strong></p></div></article><article className="rounded-2xl bg-white p-4 shadow-sm sm:p-5"><h2 className="text-xl">Customer & delivery</h2><dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2"><Info term="Name" value={order.customer.name} /><Info term="Phone" value={order.customer.normalizedPhone ?? "Not provided"} /><Info term="Email" value={order.customer.email ?? "Not provided"} /><Info term="Delivery/location" value={order.deliveryLocation ?? "Not provided"} /><Info term="Note" value={order.customerNote ?? "No note"} /><Info term="Created" value={dateTime(order.createdAt)} /><Info term="Updated" value={dateTime(order.updatedAt)} /></dl></article></div><aside className="rounded-2xl bg-asili-green p-5 text-white xl:sticky xl:top-24 xl:self-start"><h2 className="text-xl">Update progress</h2><div className="mt-6 space-y-5">{(["orderStatus", "paymentStatus", "deliveryStatus"] as StatusKind[]).map((kind) => <label key={kind} className="block text-xs font-bold uppercase tracking-wide text-white/70">{label(kind)}<select value={order[kind]} disabled={loading} onChange={(event) => void onStatus(kind, event.target.value)} className="mt-2 w-full rounded-xl bg-white px-3 py-3 text-sm font-semibold text-asili-green disabled:opacity-60">{statusOptions[kind].map((option) => <option key={option} value={option}>{label(option)}</option>)}</select></label>)}</div></aside></div></section>;
}
